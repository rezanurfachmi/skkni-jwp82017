	...
	if($_FILES['gambar']['name']!=""){
		...
		// if everything is ok, try to upload file
		    ...
		    if (move_uploaded_file($_FILES["gambar"]["tmp_name"], $target_file)) {
		        ...
		        $db->update('pesan',array(
		        							'gambar'=>$target_file
		        						),"idpesan='$id'") or eksyen('Error MySQL Pesan: Hubungi administrator','?hal=pesanan');
		        //eksyen('','?hal=produk');
		    } else {
		        eksyen('Maaf, ada kesalahan dalam unggahan gambar Anda.','?hal=pesanan');
		    }
		}
	}
		...
		<div class="row">
			<div class="col-md-3">
				...
				<div class="panel panel-primary">
					...
					<div class="panel-body">
						<p><img src="../<?=$dp['gambar'];?>" class="img-responsive"></p>
						<p><a href="cetak.php?idpesan=<?=$_GET['id'];?>" target="_blank" class="btn btn-primary btn-block"><i class="fa fa-print"></i> Cetak</a></p>
					</div>
				</div>
				<?php } ?>
			</div>
			<div class="col-md-9">
				<h1>Verifikasi Pesanan: <?=strtoupper($d['nama']);?> <small>- <a href="?hal=pesanan">Kembali</a></small></h1>
				<ol class="breadcrumb">
					...
					<div class="panel panel-primary">
						...
						<div class="panel-body">
							<div class="form-group">
								...
								<div class="form-group">
									<label for="inputUcapan" class="col-sm-2 control-label">Harga:</label>
									<div class="col-sm-9">
										<input type="text" name="ucapan" id="inputUcapan" class="form-control" value="<?=$dp['total'];?>" placeholder="Tuliskan ucapan." maxlength="50" disabled>
									</div>
								</div>
							</div>
						</div>
				...
				<p>
					<?php if($dp['verifikasi']=='0'){ ?>
					<button type="submit" class="btn btn-success"><i class="fa fa-pencil"></i> Ubah &amp; Verifikasi</button>
					<?php } ?>
